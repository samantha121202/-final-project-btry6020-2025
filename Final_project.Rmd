...
#These are the variables in the dataset I am using as well as the summary statistics for each column:
summary_table
...
pairs(data.frame(data$math.score, data$reading.score, data$writing.score))
...
data_clean$parental.level.of.education = as.factor(data_clean$parental.level.of.education)
data_clean$lunch = as.factor(data_clean$lunch)
data_clean$test.preparation.course = as.factor(data_clean$test.preparation.course)
...
data_clean$test.preparation.course = ifelse(data_clean$test.preparation.course == "completed", 1, 0)
...
#Full model
model = lm(math.score ~ gender + race.ethnicity + parental.level.of.education + lunch + test.preparation.course,
           data = data_clean)
...
bnb_model = regsubsets(math.score ~ gender + race.ethnicity + parental.level.of.education + lunch + test.preparation.course,
                           data = data_clean)
...
print("The variables included in the ideal model chosen with the branch and bound model are:")
bnb_summary$which[which.min(bnb_summary$bic), ]
#This model retains all possible variables, and is therefore identical to the forward selection model.


#Therefore, the model I will use is math.score ~ lunch + race.ethnicity + test.preparation.course + gender + parental.level.of.education.
```

```{r, warning=FALSE}
###Validate your model using an appropriate cross-validation technique and assess model performance with rmse and R2
...
p_value = summary$coefficients["parental.level.of.educationhigh school", "Pr(>|t|)"]
p_value
#Reject H0: At least one of the coefficients for parental.level.of.education is statistically significant.
...
print("Residuals appear to be slightly skewed, but probably not too much to worry about. I also tried transforming math.score in case that could further improve the normality of residuals, but all attempted transformations worsened the skewness. I tried log(), exp(), and squaring math.score.")
...
#We can use the variance inflation factor (VIF) to test for multicolinearity. VIF of 1-2 is ideal.

vif(model)

#All VIF vlaues are very close to 1, so we can conclude that the multicolinearity assumption is met for this model.
```
